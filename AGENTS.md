...
The context also carries the incoming `reqHeaders` and a once-resolved `clientIp`. tRPC procedures use those values instead of calling Next's request-scoped `headers()` API themselves. That keeps rate-limit identity consistent across search, newsletter, and sign-in, passes the original headers to Better Auth, and lets `createAnonymousCaller()` execute the same procedures in-process with an empty header set (`clientIp === "unknown"`).
...
Zustand store (`src/lib/stores/cart-store.ts`) persisted to localStorage, hydrated from the server by `CartProvider`. `useCart()` in `src/components/providers/cart-provider.tsx` is the API components should use — it wraps the store with server mutations, optimistic updates, and a 1s debounce on quantity changes.

**Guests do have a cart.** This file said "guests cannot add to cart; `addItem` shows a sign-in toast instead" for a long time and it was false — found stale while adding the coupon hold. `addItem` writes a `guest-` prefixed line straight into the local store (it needs `guestDetails` to have anything to display, and only errors if a call site forgot to pass it); `public.cart.mergeGuestItems` folds those lines into the server cart at sign-in. The merge is keyed off the id **prefix**, not off an unauthenticated→authenticated transition, because login does a full-page navigation that remounts the provider and loses any transition tracked in a ref.
...
**The cart is a row now.** `carts` owns `cart_items` through `cart_id`; before this it was an implicit collection, a `WHERE user_id =` over `cart_items`. `CartRepositoryInterface` is still keyed on `userId` and resolves the cart id internally, so no caller has ever seen one. `carts.user_id UNIQUE` is the whole of "one cart per user" — dropping that constraint is the path to saved or multiple carts, and nothing else has to change for it.

**An applied coupon reserves nothing.** It is remembered, not held: `carts.coupon_id` records which code the customer typed, the cart shows the **code** and never computes or displays a discount amount (checkout prices it), and the guarded `UPDATE` in `order.repository.ts` remains the only authority on usage and per-customer limits. Re-validation is lazy — on read, at most once every 15 minutes (`COUPON_RECHECK_MS` in `src/lib/cart-coupon-freshness.ts`), skipped entirely on an empty cart because a minimum-purchase rule would reject any code against one. `createSession` / `createCodOrder` no longer take a `couponCode` input at all: the cart is the single source of truth and the client no longer gets a say in which code it is charged against.

**A held coupon is dropped only for reasons that are properties of the coupon.** `not_found`, `inactive`, `not_yet_valid`, `expired`, `usage_limit`, `user_limit` — those describe a code that is dead, so `GetCartUseCase` clears it and tells the customer which code and why. `below_minimum` and `pending_order` describe the **cart right now**, resolve on their own when the cart changes, and are kept. That is what `reason` and `isTransientCouponRejection` in `validate-coupon.use-case.ts` exist for. The earlier version dropped on any `valid: false`, which silently and irreversibly deleted a perfectly good coupon the first time the cart emptied or dipped below the minimum for fifteen minutes; the customer got no way back except retyping a code they had no reason to think was gone. **The trap for whoever edits it next:** a new rejection branch added without a `reason` defaults to "dead coupon" and deletes the customer's code. `validate-coupon.use-case.test.ts` asserts the exact tag on every branch, which is what makes that mistake loud.
